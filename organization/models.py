...
class Organization(models.Model):
    ...
    organization_product = models.ManyToManyField('organization.OrganizationProduct', blank=True, null=True)
...
class OrganizationProduct(models.Model):
    ...
    product_suggestion = models.ManyToManyField('product.Product')

    def __str__(self):
        return f'{self.name}'

    def save(self, *args, **kwargs):
        print(self.product_suggestion)
        if not self.id:
            self.slug = slugify(self.name, allow_unicode=True)
        super().save(*args, **kwargs)
